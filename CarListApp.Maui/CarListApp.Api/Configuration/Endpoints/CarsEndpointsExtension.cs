...
internal static class CarsEndpointsExtension
{
    internal static void RegisterCarEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapPut("/cars/{id}", async (IMediator _mediator, int id, Car car) => await _mediator.Send(new UpdateCarCommand(id, car.Make, car.Model, car.Vin))
                ? Results.NoContent()
                : Results.NotFound());

        endpoints.MapDelete("/cars/{id}", async (IMediator _mediator, int id) => await _mediator.Send(new DeleteCarCommand { Id = id }) 
                ? Results.NoContent() 
                : Results.NotFound());
